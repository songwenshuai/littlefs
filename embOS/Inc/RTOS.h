...
#define OS_TIME_GetTicks()                   (OS_Global.Time)  // OS_Global.Time is declared as volatile
...
#define OS_IDLE()                            return

#if (defined(_MSC_VER) && (_MSC_VER <= 1200))
  #define OS_I64  __int64
#else
  #define OS_I64  long long
#endif

#if (defined(_WIN64) && (_WIN64 == 1))
  #define OS_TIME       OS_I64
...
//
// Renames main to _main. Else the MinGW gcc compiler would call main instead of WinMain.
//
#ifdef __MINGW32__
...
void  OS_SIM_MarkInISR        (void);
void  OS_SIM_EI_On_Leave      (void);
void* OS_SIM_CreateISRThread  (OS_ISR_HANDLER* pfStartAddress);
void* OS_SIM_CreateISRThreadEx(OS_ISR_HANDLER* pfStartAddress, const char* sThreadName);
void  OS_SIM_DeleteISRThread  (void* hThreadHandle);
void  OS_SIM_EnterSysCall     (void);
void  OS_SIM_LeaveSysCall     (void);
void  OS_SIM_SetThreadName    (unsigned long ThreadID, const char* sThreadName);
int   main                    (void);

//
// Using functions instead of inlined code to facilitate debug ability
//
#define OS_EI_ON_LEAVE()  OS_SIM_EI_On_Leave()
#define OS_MARK_IN_ISR()  OS_SIM_MarkInISR()
